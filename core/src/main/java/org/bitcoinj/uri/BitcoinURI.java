...
/**
 * <p>Provides a standard implementation of a dobbscoin URI with support for the following:</p>
 *
 * <ul>
 ...
 *
 * <ul>
 * <li>{@code dobbscoin:<address>}</li>
 * <li>{@code dobbscoin:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 * @see <a href="https://github.com/dobbscoin/bips/blob/master/bip-0021.mediawiki">BIP 0021</a>
 */
public class BitcoinURI {
    ...
    /**
     * URI for dobbscoin network. Use {@link org.bitcoinj.params.AbstractBitcoinNetParams#BITCOIN_SCHEME} if you specifically
     * need dobbscoin, or use {@link org.bitcoinj.core.NetworkParameters#getUriScheme} to get the scheme
    ...
    /**
     * Constructs a new object by trying to parse the input as a valid dobbscoin URI.
     *
     ...
     *
     * @throws BitcoinURIParseException If the input fails dobbscoin URI syntax and semantic checks.
     */
    public BitcoinURI(@Nullable NetworkParameters params, String input) throws BitcoinURIParseException {
        ...
        // URI is formed as  dobbscoin:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form dobbscoin://address?....
        // We support both until Ben fixes his code.
        
        // Remove the dobbscoin scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : dobbscoin:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        ...
        if (addressSplitTokens.length == 0)
            throw new BitcoinURIParseException("No data found after the dobbscoin: prefix");
        ...
        if (!addressToken.isEmpty()) {
            // Attempt to parse the addressToken as a dobbscoin address for this network
            try {
    ...
    private void parseParameters(@Nullable NetworkParameters params, String addressToken, String[] nameValuePairTokens) throws BitcoinURIParseException {
        ...
        for (String nameValuePairToken : nameValuePairTokens) {
            ...
            if (sepIndex == -1)
                throw new BitcoinURIParseException("Malformed dobbscoin URI - no separator in '" +
                        nameValuePairToken + "'");
            if (sepIndex == 0)
                throw new BitcoinURIParseException("Malformed dobbscoin URI - empty name '" +
                        nameValuePairToken + "'");
    ...
    /**
     * The dobbscoin Address from the URI, if one was present. It's possible to have dobbscoin URI's with no address if a
    ...
    /**
     * Simple dobbscoin URI builder using known good fields.
     * 
     * @param address The dobbscoin address
     * @param amount The amount
     * @param label A label
     * @param message A message
     * @return A String containing the dobbscoin URI
     */
