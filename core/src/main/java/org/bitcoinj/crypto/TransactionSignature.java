...
public class TransactionSignature extends ECKey.ECDSASignature {
     ...
     * and generally violates people's mental model of how dobbscoin should work, thus, non-canonical signatures are now
     ...
     * of the type used by dobbscoin we have to encode them using DER encoding, which is just a way to pack the two
    ...
    public static TransactionSignature decodeFromBitcoin(byte[] bytes, boolean requireCanonical) throws VerificationException {
        // dobbscoin encoding is DER signature + sighash byte.
        ...
        // In dobbscoin, any value of the final byte is valid, but not necessarily canonical. See javadocs for
