...
/**
 * <p>A Peer handles the high level communication with a dobbscoin node, extending a {@link PeerSocketHandler} which
...
public class Peer extends PeerSocketHandler {
        ...
        try {
            ...
            try {
                if (awaitingFreshFilter != null) {
                    ...
                    // range of blocks and get an inv both times, due to the codepath in dobbscoin Core hitting
    ...
    private void blockChainDownloadLocked(Sha256Hash toHash) {
        ...
        //
        //   https://en.dobbscoin.it/wiki/Protocol_specification#getblocks
        //
     ...
     * after the query returns. <b>Most peers do not support this request. You will need to connect to dobbscoin XT
    ...
    public ListenableFuture<UTXOsMessage> getUTXOs(List<TransactionOutPoint> outPoints) {
        ...
        try {
            ...
            if ((peerVer.localServices & GetUTXOsMessage.SERVICE_FLAGS_REQUIRED) != GetUTXOsMessage.SERVICE_FLAGS_REQUIRED)
                throw new ProtocolException("Peer does not support getutxos protocol flag: find dobbscoin XT nodes.");
